...
To upload a file, click the button and choose a file or drag&drop a file on the button. Files are stored in a folder in the data directory (see [Customizing the data directory](https://docs.geonetwork-opensource.org/4.4/install-guide/customizing-data-directory/)). There is one folder per metadata containing:
