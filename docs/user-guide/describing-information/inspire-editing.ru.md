...
Идентификатор новой записи автоматически устанавливается каталогом. Дата последнего обновления метаданных также автоматически обновляется каждый раз, когда редактор сохраняет запись.
...
Информация о контакте требует заполнения 2-х обязательных полей: `название организации` и `электронная почта`. **Требуется как минимум один пункт в контакте для ресурса и один для метаданных.**
...
При добавлении ключевого слова каталог предлагает настроить способ записи значения в XML-документе. По умолчанию используется текстовая запись:
...
Способ записи ключевого слова сохраняется после установки и может быть разным для разных записей. В редакторе ее можно установить глобально, задав параметр `transformations` для данного тезауруса:
``` xml
<thesaurusList>
...
Под ограничением ресурсам подразумевается его ограничения на распространения (например авторские права). Ограничения ресурсов должны быть установлены из кодового списка [Limitation on public access](https://inspire.ec.europa.eu/metadata-codelist/LimitationsOnPublicAccess):
...
Во время редактирования пользователь может вызвать проверку на соответсвие INSPIRE с помощью [общепринятого валидатора](https://inspire.ec.europa.eu/validator/). Каталог отправит запись валидатору и отобразит ссылку на отчет после завершения проверки:
...
Также можно использовать [последний стандарт ISO ISO19115-3:2018](https://github.com/metadata101/iso19115-3.2018) для кодирования наборов данных INSPIRE. В этом случае перед проверкой запись преобразуется обратно в ISO19139. Такой подход позволяет воспользоваться улучшениями, предоставляемыми стандартом, и сохранить соответствие требованиям INSPIRE.
