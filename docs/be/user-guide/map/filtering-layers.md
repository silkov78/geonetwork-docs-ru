...
* Трэцяе поле ўводу (у выпадку палёў тыпу String) прадастаўляе пастаронкавы спіс даступных значэнняў поля, якія ўжо прысутнічаюць у наборы дадзеных слоя (для гэтага выкарыстоўваецца працэс WPS GeoServer). У выпадку лікавых палёў карыстальнік можа проста ўвесці значэнне для выкарыстання ў фільтры.
