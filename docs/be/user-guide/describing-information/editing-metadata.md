# Рэдагаванне метададзеных

У гэтым раздзеле апісваецца, як выкарыстоўваць рэдактар метададзеных.

## Выкарыстанне панэлі інструментаў рэдактара

Праз `Рэдагаванне` - `Панэль рэдактара` (або праз `Пошук`) знайдзіце запіс метададзеных, які трэба змяніць, і адкрыйце яго. Клікніце на кнопку `Рэдагаваць`, каб адкрыць старонку рэдагавання запісу. Верхняя панэль інструментаў прадастаўляе найбольш важныя функцыі, даступныя рэдактару:

![](img/editor-toolbar.ru.png)

Злева направа:

- `Катэгорыі` для выбару катэгорыі ў метададзеных;
- `Група` для выбару групы метададзеных;
- `Праверыць` для запуску праверкі запісу метададзеных на адпаведнасць стандарту;
...
- `Захаваць` для захавання метададзеных або шаблона. Выпадальны спіс таксама дазваляе пазначыць, ці з'яўляюцца праўкі нязначнымі (г.зн. дата змянення метададзеных не будзе абнаўляцца ў выпадку нязначных правак);
- `Рэжым прагляду` для:
...
- `Па змаўчанні` адлюстроўвае ўсе абавязковыя палі для выкарыстоўванага стандарту метададзеных;
- `Падрабязны выгляд` адлюстроўвае ўсе абавязковыя і **ўсе апцыянальныя** палі для выкарыстоўванага стандарту метададзеных;
