# Збор метададзеных з каталогаў GeoNetwork версій 2.1-3.X

Для ажыццяўлення збору запісаў метададзеных з каталогаў GeoNetwork версій 2.1-3.X рэалізаваны асобны зборшчык - ён падключаецца да аддаленага сервера GeoNetwork, і атрымлівае запісы метададзеных, якія адпавядаюць параметрам запыту.

# Стварэнне зборшчыка 

Пераход да стварэння зборшчыка метададзеных з базы дадзеных ажыццяўляецца праз варыянт `Збор дадзеных` панэлі `Адміністраванне`. У акне, якое адкрылася, у крыніцы `Сабраць з` варта выбраць опцыю `GeoNetwork (from 2.1 to 3.x)`.
