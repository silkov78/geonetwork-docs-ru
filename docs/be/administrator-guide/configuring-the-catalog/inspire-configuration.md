...
Для наладкі службы выяўлення НЕАБХОДНА стварыць спецыяльны запіс метаданых службы, 
...
Для апісання набораў даных і серый INSPIRE рэкамендуецца загружаць адпаведныя спісы кодаў 
з [Рэестра INSPIRE](https://inspire.ec.europa.eu/registry/), наступныя спісы кодаў адпавядаюць патрабаванням кіраўніцтва па метаданых версіі 2.0:
...
- Рэестр кодавых даных -> [Пратаколы](https://inspire.ec.europa.eu/metadata-codelist/ProtocolValue)
- Рэестр кодавых даных -> [Прасторавы ахоп](https://inspire.ec.europa.eu/metadata-codelist/SpatialScope)
- Рэестр кодавых спісаў метаданых --> [Набор прыярытэтных даных INSPIRE](https://inspire.ec.europa.eu/metadata-codelist/PriorityDataset)
- Рэестр кодавых даных метаданых -> [Катэгорыя службы прасторавых даных](https://inspire.ec.europa.eu/metadata-codelist/SpatialDataServiceCategory)
- Рэестр кодавых даных метаданых -> [Умовы доступу і выкарыстання](https://inspire.ec.europa.eu/metadata-codelist/ConditionsApplyingToAccessAndUse)
- Рэестр кодавых даных метаданых -> [Абмежаванні на публічны доступ](https://inspire.ec.europa.eu/metadata-codelist/LimitationsOnPublicAccess)
- Зарэгістраваць спіс кодаў метаданых --> [Анлайн-код апісання](https://inspire.ec.europa.eu/metadata-codelist/OnLineDescriptionCode)
- Рэестр кодавых спісаў метаданых -> [Крытэрыі якасці абслугоўвання](https://inspire.ec.europa.eu/metadata-codelist/QualityOfServiceCriteria)
...
Пасля загрузкі тэзаўрус можна выкарыстоўваць у запісах метаданых для выбару ключавых слоў з:
...
Праверка запісаў метаданых INSPIRE даступная па адрасе [the INSPIRE Validator](https://inspire.ec.europa.eu/validator/about/). 
У ім выкарыстоўваецца [ETF, які з'яўляецца платформай тэсціравання прасторавых даных і сэрвісаў з адкрытым зыходным кодам](https://github.com/etf-validator/etf-webapp). GeoNetwork можа "апрацаваць" любы запіс, выкарыстоўваючы сэрвіс, які прадастаўляецца экзэмплярам ETF. Каб наладзіць аддаленую праверку, перайдзіце ў "Кансоль адміністратара" -> "Наладкі" і ўкажыце URL-адрас сродку праверкі. URL-адрас асноўнага сродку праверкі INSPIRE - гэта `https://inspire.ec.europa.eu/validator/`.
...
Для кожнага запісу вы можаце вызначыць умову XPath, якую павінны выконваць метаданыя для адпраўкі валідатару.

!!! note

    Калі схема метаданых не адпавядае, правяраецца іерархія залежнасцей схемы, 
...
!!! warning
    ...
    будзе інтэрпрэтавацца GeoNetwork як заўсёды супадаючы.
...
У многіх выпадках толькі частка запісаў метаданых у каталогу звязана з дырэктывай INSPIRE. 
...
- Дадайце канкрэтнае ключавое слова ў запіс метаданых.
