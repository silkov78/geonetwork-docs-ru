...
Каталог падтрымлівае запісы метададзеных, якія складаюцца з фрагментаў метададзеных. 
Ідэя заключаецца ў тым, што фрагменты метададзеных могуць выкарыстоўвацца больш чым у адным запісе метададзеных.

Вось тыповы прыклад фрагмента. Гэта адказны бок, і ён можа выкарыстоўвацца ў адным і 
тым жа запісе метададзеных больш за адзін раз ці ў некалькіх запісах метададзеных, калі гэта дастасавальна.
...
Фрагменты метададзеных, якія захоўваюцца ў базе дадзеных каталога, называюцца **падшаблонамі**. 
У асноўным гэта робіцца па гістарычных прычынах, бо падшаблон падобны да шаблоннага запісу метададзеных, 
паколькі ён можа быць выкарыстаны ў якасці «шаблона» для стварэння новага запісу метададзеных.

Фрагменты могуць быць устаўленыя ў запіс метададзеных двума спосабамі:
...
Пры выкарыстанні XLinks, калі фрагмент абнаўляецца, то звязаны з ім фрагмент ва ўсіх запісах метададзеных таксама будзе абноўлены (праверце кэш XLink).

Фрагменты могуць быць створаны шляхам збору 
(гл. [Збор фрагментаў метададзеных для падтрымкі паўторнага выкарыстання](../../user-guide/harvesting/index.md#збор-фрагментаў-метаданых-для-паўторнага-выкарыстання)) 
або імпартаваны з дапамогай старонкі імпарту метададзеных.

У гэтым раздзеле кіраўніцтва апісана:

- як кіраваць каталогамі падшаблонаў
- як вымаць фрагменты з існуючага набору запісаў метададзеных і захоўваць іх у выглядзе падшаблонаў
...
Існуюць некаторыя адрозненні паміж працай з падшаблонамі і запісамі метададзеных. 
У адрозненне ад запісаў метададзеных, падшаблоны не маюць узгодненага каранёвага элемента, 
схема метададзеных, якую яны выкарыстоўваюць, можа быць нераспазнавальнай, яны не з'яўляюцца ў асноўных выніках пошуку 
(калі толькі не з'яўляюцца часткай запісу метададзеных). Таму панэль рэдактара дазваляе шукаць і кіраваць прывілеямі для запісаў каталогаў.
...
Калі ўкладку `Арганізацыі і кантакты` не відаць, пераканайцеся, што былі створаны падшаблоны для кантактаў для свайго профілю метададзеных 
...
Для імпарту новых запісаў выкарыстоўвайце старонку імпарту метададзеных і выберыце адпаведны тып запісу:

![](img/directories-import.png)

Як і запісам метададзеных, ім прысвойваецца цэлалікавы ідэнтыфікатар, і яны захоўваюцца ў табліцы метададзеных каталога (з полем шаблона, усталяваным на «y»).
## Устаўце запіс каталога ў запіс метададзеных

У рэдактары метададзеных каталог можна выкарыстоўваць, напрыклад, для запаўнення кантактаў.
...
## Выманне ўкладзеных шаблонаў з запісаў метададзеных

На многіх сайтах ужо існуюць запісы метададзеных з агульнай інфармацыяй, напрыклад, кантактная інфармацыя ў элеменце ISO CI_Contact. 
Запісы каталога, падобныя да гэтых, могуць быць вынятыя з выбранага набору запісаў метададзеных з дапамогай API "Extract subtemplates".
...
- Вызначыць фрагменты метададзеных, якімі яны хацелі б кіраваць як падтэмплатамі паўторнага выкарыстання ў запісе метададзеных. 
...
## Сінхранізацыя падшаблонаў з запісамі метададзеных

Пасля стварэння каталог прадастаўляе магчымасць сінхранізаваць запісы метададзеных з запісамі каталога. Для гэтага выкарыстоўвайце старонку тэсціравання API.

Працэс сінхранізацыі выкарыстоўвае тыя ж параметры, што і працэс збору, з двума дадатковымі аргументамі:

- `propertiesToCopy` для захавання некаторага элемента, які можа быць вызначаны ў фрагменце ў метададзеных (напрыклад, роля кантакту)
...
Калі запісы метададзеных у вашым каталогу ўтрымліваюць фрагменты спасылак са знешніх сайтаў, 
каталог кэшуе гэтыя фрагменты пасля першага пошуку, каб паменшыць аб'ём сеткавага трафіку 
і паскорыць адлюстраванне запісаў метададзеных у выніках пошуку.
...
Некаторыя аперацыі ў каталогу (напрыклад, збор ураджаю), якія генеруюць фрагменты метададзеных, 
