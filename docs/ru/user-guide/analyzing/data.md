...
!!! note "Примечание"
    ...
    В качестве поисковой системы в Каталоге метаданных используется [Elasticsearch](https://www.elastic.co/elasticsearch) - одна из самых популярных поисковых систем в области Big Data, масштабируемое нереляционное хранилище данных с открытым исходным кодом, аналитическая NoSQL-СУБД с широким набором функций полнотекстового поиска.

Когда информация о данных будет получена с WFS-сервера, становится доступна фильтрация. 
Интерфейс фильтрации предоставляет возможность разбиения набора данных на группы (фасетирования), а также возможность экспорта. Вьюер карт пытается подключиться к службе WFS, заменяя в URL службы WMS на WFS. Если такой запрос на WFS-сервис успешный, панель фильтрации предоставляет возможность индексировать объекты у рассматриваемого набора данных. Пользователь должен быть авторизован, чтобы запустить процесс индексирования.
