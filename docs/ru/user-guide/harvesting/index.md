...
!!! note "Примечание"
    ...
    Если пользователь по каким-то причинам намерен редактировать собранные записи метаданных и его не беспокоят возможные проблемы, описанные выше, существует настройка, которая позволит это сделать. См. [Импорт метаданных](../../administrator-guide/configuring-the-catalog/system-configuration.md#импорт-метаданных) для получения более подробной информации.
...
## Сбор фрагментов метаданных для повторного использования
...
8. *Операция* Список кнопок/ссылок на операции для сборщика.
   ...
   - Выбор *История* позволит вам просмотреть/изменить историю сбора данных для сборщика - см. [История сбора данных](index.md#история-сбора-данных).
...
4. *История* Показывает историю харвестеров всех харвестеров. Подробнее см. [История харвестеров](index.md#история-сбора-данных).
