...
* С помощью инструмента *Фильтр слоя*, доступного в [Панели содержания](toc.md#table-of-contents)

* С помощью инструмента *Расширенный поиск*, доступного из [Таблицы атрибутов](attributes-table.md#attribute-table)

* С помощью *Быстрого фильтра*, доступного в [Таблице атрибутов](attributes-table.md#attribute-table)

### Фильтр слоя

Этот фильтр применяется с помощью кнопки **Фильтр слоя** <img src="../img/button/filter-layer.jpg" class="ms-docbutton"/> на [Панели инструментов слоев](toc.md#toolbar-options) в Панели содержания и будет сохраняться в следующих ситуациях:

* При использовании других инструментов, таких как [Инструмент "Идентификация"](navigation-toolbar.md#identify-tool):
...
* При следующем открытии карты (необходимо Сохранить карту с [Боковой панели инструментов](mapstore-toolbars.md#side-toolbar) после применения фильтра)

После установки *Фильтра слоя* его можно включить/отключить, просто нажав на кнопку, которая появится рядом с названием слоя в [Панели содержания](toc.md#table-of-contents):
...
Этот фильтр, применяемый с помощью кнопки **Расширенный поиск** <img src="../img/button/advanced-search.jpg" class="ms-docbutton"/> в [Таблице атрибутов](attributes-table.md#attribute-table), ведет себя следующим образом:

* Его можно использовать для применения фильтра к слою для поиска в [Таблице атрибутов](attributes-table.md#attribute-table): этот фильтр применяется по условию `AND` к *Фильтру слоя*, если он уже установлен.
...
* Он будет автоматически удаляться/применяться заново при закрытии/открытии [Таблицы атрибутов](attributes-table.md#attribute-table)
...
Можно фильтровать записи в [Таблице атрибутов](attributes-table.md#attribute-table), щелкая по карте или делая выборку нескольких объектов непосредственно на карте. Пользователь может активировать кнопку **Фильтр на карте** <img src="../img/button/filter_geometry_button.jpg" class="ms-docbutton"/> (после нажатия кнопка становится синей), а затем:
...
Из [Таблицы атрибутов](attributes-table.md#attribute-table) пользователь может фильтровать данные по видимому экстенту карты с помощью кнопки **Фильтр по экстенту** <img src="../img/button/filter-by-viewport-button.jpg" class="ms-docbutton"/>. После нажатия кнопка-переключатель становится зеленой, и список записей в *Таблице атрибутов* фильтруется, показывая только записи, соответствующие объектам слоя, присутствующим в текущем видимом экстенте карты.
...
!!! Внимание
    *Быстрый фильтр* остается активным, пока открыта [Таблица атрибутов](attributes-table.md#attribute-table), но, в отличие от *Расширенного поиска*, после закрытия [Таблицы атрибутов](attributes-table.md#attribute-table) он больше не появится, если [Таблица атрибутов](attributes-table.md#attribute-table) будет открыта повторно.
...
Этот фильтр позволяет установить одно или несколько условий, относящихся к полям [Таблицы атрибутов](attributes-table.md#attribute-table).
...
* *Целевой слой* (из числа присутствующих в [Панели содержания](toc.md#table-of-contents))
